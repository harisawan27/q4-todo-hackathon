"""Async Dapr HTTP client wrapper for recurring-engine with Jobs API support."""
...
DAPR_HTTP_PORT = int(os.environ.get("DAPR_HTTP_PORT", "3502"))
...
async def register_job(job_name: str, job_config: dict[str, Any]) -> None:
    """Register a job with the Dapr Jobs API (alpha1)."""
    async with httpx.AsyncClient() as client:
...
async def delete_job(job_name: str) -> None:
...
async def get_job(job_name: str) -> dict[str, Any] | None:
