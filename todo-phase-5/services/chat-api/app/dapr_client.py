"""Async Dapr HTTP client wrapper for chat-api service."""
...
async def save_state(items: list[dict[str, Any]]) -> None:
    """Save one or more state items. Each item: {key, value, etag?, options?}."""
    async with httpx.AsyncClient() as client:
        resp = await client.post(
...
async def transact_state(operations: list[dict[str, Any]]) -> None:
    """Execute a state transaction (atomic upserts and deletes)."""
    async with httpx.AsyncClient() as client:
        resp = await client.post(
